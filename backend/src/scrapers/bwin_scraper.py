"""
Bwin.pt Scraper - PARSER-BACKED FALLBACK IMPLEMENTATION

This module provides Bwin Portugal support in the same safe mode as the existing
bookmaker scrapers:
1. JSON parser helpers for representative Bwin sportsbook payload shapes.
2. Conservative fallback mock data when live scraping is unavailable.

Live production scraping is intentionally not guaranteed here because Bwin.pt may
...
class BwinScraper(BaseScraper):
    """Scraper for Bwin.pt with parser helpers and safe fallback semantics."""

    def __init__(self):
        config = BETTING_SITES.get("bwin", {})
        super().__init__(
            site_key="bwin",
            site_name=config.get("name", "Bwin.pt"),
    ...
    def get_upcoming_matches(self, days_ahead: int = 7) -> List[OddsData]:
        """Get upcoming Bwin matches or deterministic fallback matches."""
        try:
            ...
            print(f"⚠️  Bwin scraping not available, using mock: {e}")
        except Exception as e:
            print(f"⚠️  Bwin scraping failed, using mock: {str(e)}")

        return self._get_mock_upcoming_matches(days_ahead)

    def parse_upcoming_matches_json(self, payload: str) -> List[OddsData]:
        """
        Parse representative Bwin sportsbook JSON into normalized OddsData rows.
    ...
    def _iter_event_nodes(
        self, node: Any, context: Optional[Dict[str, Any]] = None
    ) -> Iterable[Dict[str, Any]]:
        """Yield dicts that look like Bwin event nodes from nested JSON."""
        context = context or {}
        if isinstance(node, list):
            for item in node:
                yield from self._iter_event_nodes(item, context)
            return
        ...
        if isinstance(competition, dict) and competition.get("name"):
            next_context["competition_name"] = competition["name"]
        elif node.get("competitionName"):
            next_context["competition_name"] = node["competitionName"]

        region = node.get("region")
        if isinstance(region, dict) and region.get("name"):
            next_context["region_name"] = region["name"]
        elif region:
            next_context["region_name"] = region
        elif node.get("category"):
            next_context["region_name"] = node["category"]
        ...
        for key in ("events", "fixtures", "items", "children", "widgets", "data"):
            ...
            if child is not None:
                yield from self._iter_event_nodes(child, next_context)
    ...
    def _parse_event(self, event: Dict[str, Any]) -> Optional[OddsData]:
        ...
        match_id = str(
            ...
            or f"bwin_{home_team}_{away_team}_{match_date.strftime('%Y%m%d')}"
        )
    ...
    def _create_mock_odds(
        ...
        """Create deterministic fallback odds data; not real Bwin.pt odds."""
        if match_date is None:
        ...
        return OddsData(
            match_id=f"bwin_{home_team}_{away_team}_{match_date.strftime('%Y%m%d')}",
            home_team=home_team,
            away_team=away_team,
            match_date=match_date,
            site="bwin",
            ...
            btts_yes=1.62,
            btts_no=1.92,
            url=f"{self.base_url}/sports/futebol/{home_team}-{away_team}",
            market_name="1x2",
            status="fallback",
            error="Live Bwin.pt scrape unavailable; deterministic fallback odds used.",
            source_url=f"{self.base_url}/sports/futebol/{home_team}-{away_team}",
        )
