...
class PlacardScraper(BaseScraper):
    ...
    def parse_upcoming_matches_json(self, payload: str) -> List[OddsData]:
        ...
        for event in events:
            ...
            match_id = (
                ...
                or self._build_match_id(home_team, away_team, match_date)
            )
            status = event.get("status") or event.get("state") or "upcoming"
    ...
    def _extract_event_url(self, event: Dict[str, Any]) -> str:
        ...
        return urljoin("https://apostas.placard.pt", str(source))
