...
class TestDataLoader:
    ...
    def test_cache_odds_replaces_existing(self, data_loader, sample_odds):
        ...
        cursor.execute("SELECT COUNT(*) FROM odds_cache WHERE match_id = ?", 
                      ("test_match_123",))
        count = cursor.fetchone()[0]
        
        assert count == 1
        
        cursor.execute("SELECT home_win FROM odds_cache WHERE match_id = ?",
                      ("test_match_123",))
    ...
    def test_log_prediction(self, data_loader):
        ...
        cursor.execute(
            "SELECT home_team, away_team, home_prob FROM predictions_log"
        )
        row = cursor.fetchone()
