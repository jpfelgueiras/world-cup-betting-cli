...
class DataLoader:
    ...
    def get_cached_odds(
        self, home_team: str, away_team: str, max_age_hours: int = 1
    ) -> List[Any]:
    ...
    def log_prediction(
        ...
        """,
            (
                ...
                away_team,
                (
                    prediction.match_date.isoformat()
                    if hasattr(prediction, "match_date")
                    else None
                ),
...
class FBrefLoader:
    ...
    def get_head_to_head(self, team_a: str, team_b: str) -> Dict[str, int]:
